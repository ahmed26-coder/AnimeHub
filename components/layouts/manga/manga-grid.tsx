...
import { Star, BookOpen, Users, ChevronLeft, ChevronRight, Heart } from "lucide-react"
...
export function MangaGrid() {
  ...
  useEffect(() => {
    const savedLikes = localStorage.getItem("likedManga")
    if (savedLikes) {
  ...
  useEffect(() => {
    localStorage.setItem("likedManga", JSON.stringify(likedItems))
  }, [likedItems])
        ...
        {manga.map((item) => {
          ...
          return (
            <Link key={item.mal_id} href={`/manga/${item.mal_id}`}>
              <Card className=" py-0 h-full overflow-hidden hover:shadow-lg transition-shadow group">
